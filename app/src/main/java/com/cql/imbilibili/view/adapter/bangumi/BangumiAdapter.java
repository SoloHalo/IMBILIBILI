...
import com.cql.imbilibili.utils.transformation.RoundedCornersTransformation;
import com.cql.imbilibili.widget.BannerView;
...
public class BangumiAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public static final int COUNTRY_BANGUMI_HEAD = 4;
    public static final int COUNTRY_BANGUMI_ITEM = 5;
    public static final int SEASON_BANGUMI_HEAD = 6;
    public static final int SEASON_BANGUMI_ITEM = 7;
    public static final int BANGUMI_RECOMMEND_HEAD = 8;
    public static final int BANGUMI_RECOMMEND_ITEM = 9;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == BANNER) {
            ...
            View gridView = inflater.inflate(R.layout.bangumi_grid_item, parent, false);
            holder = new SeasonHolder(gridView);
        } else if (viewType == COUNTRY_BANGUMI_HEAD) {
            View headView = inflater.inflate(R.layout.common_head_item, parent, false);
            holder = new HeadHolder(headView);
        } else if (viewType == COUNTRY_BANGUMI_ITEM) {
            View gridView = inflater.inflate(R.layout.bangumi_grid_item, parent, false);
            holder = new SeasonHolder(gridView);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (getItemViewType(position) == SERIALIZING_HEAD) {
            ...
            if (realPosition < bangumis.size()) {
                ...
                seasonHolder.tvFavourite.setVisibility(View.GONE);
                seasonHolder.tv1.setVisibility(View.VISIBLE);
                seasonHolder.tvTitle.setText(bangumi.getTitle());
                seasonHolder.tv1.setText(StringUtils.format("更新至第%s话", bangumi.getNewestEpIndex()));
                seasonHolder.tvFavourite.setText(bangumi.getLastTime());
                Drawable drawable = DrawableCompat.wrap(seasonHolder.tvFavourite.getBackground());
                ...
                seasonHolder.tvFavourite.setBackground(drawable);
                seasonHolder.tvFavourite.setText(StringUtils.format("%s人在看", StringUtils.formateNumber(bangumi.getWatchingCount())));
                seasonHolder.mSeasonId = bangumi.getSeasonId();
            } else {
                seasonHolder.itemView.setVisibility(View.GONE);
            }
        } else if (getItemViewType(position) == COUNTRY_BANGUMI_HEAD) {
            HeadHolder headHolder = (HeadHolder) holder;
            headHolder.tvTitle.setText("国产动画");
            headHolder.setLeftDrawable(R.mipmap.ic_category_t153);
            headHolder.tvSubTitle.setText("更多");
        } else if (getItemViewType(position) == COUNTRY_BANGUMI_ITEM) {
            SeasonHolder seasonHolder = (SeasonHolder) holder;
            List<Bangumi> bangumis = mIndexPage.getChina();
    ...
    public int getItemCount() {
        ...
        if (mIndexPage != null) {
            ...
            if (mIndexPage.getChina() != null) {
                mTypeList.add(COUNTRY_BANGUMI_HEAD);
                mTypeList.add(COUNTRY_BANGUMI_ITEM);
                mTypeList.add(COUNTRY_BANGUMI_ITEM);
                mTypeList.add(COUNTRY_BANGUMI_ITEM);
            }
        }
