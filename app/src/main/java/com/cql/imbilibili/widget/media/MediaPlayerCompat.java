...
public class MediaPlayerCompat {
    ...
    public static String getPlayerVideoDecodeType(IMediaPlayer mp, Context context) {
        StringBuilder builder = new StringBuilder();
        if(mp ==null) {
            builder.append(context.getString(R.string.N_A));
        } else{
            builder.append(mp.getMediaInfo().mVideoDecoder);
            if(mp instanceof AndroidMediaPlayer){
                builder.append(":");
            }else {
                builder.append("\n+");
            }
            builder.append(mp.getMediaInfo().mVideoDecoderImpl);
        }
        return builder.toString();
    }

    public static String getPlayerAudioDecodeType(IMediaPlayer mp,Context context){
        StringBuilder builder = new StringBuilder();
        if(mp == null){
            builder.append(context.getString(R.string.N_A));
        }else{
            builder.append(mp.getMediaInfo().mAudioDecoder);
            builder.append(":");
            builder.append(mp.getMediaInfo().mAudioDecoderImpl);
        }
        return builder.toString();
    }
